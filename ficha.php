...
<body>
	...
	<div class="container-fluid pt-0 px-0 pt-2" id="containerPadre">
		...
		<?php if (!isset($_GET['cursor'])) { ?>
				...
				<div class="row">
					...
					<div class="table-responsive">
						<table class="table table-hover">
							...
							/** @var mysqli $cadena */
							$sqlInmueble = "SELECT `idFicha`, lower(`fichTitulo`) as fichTitulo, `fichPrecio`, `moneda` FROM `fichas` WHERE `fichActivo`=1 order by trim(fichTitulo) asc; ";
							$resultadoInmueble = $cadena->query($sqlInmueble);
							$i = 1; ?>
									...
									<tr>
										...
										<td style="white-space: nowrap;">
											...
											<?php if ($_COOKIE['ckPower'] == 1): ?>
												...
												<button class="btn button btn-outline-danger btn-sm" title="Eliminar" onclick="borrarFicha(<?= $rowInmueble['idFicha']; ?>);"><i class="bi bi-eraser"></i></button>
											<?php endif; ?>
										</td>
									</tr>
								<?php $i++;
			...
			$sql = "SELECT `idFicha`, `fichTitulo`, `fichPrecio`, `moneda`, `fichDireccion`, `fichTipoPropiedad`, `fichAreaTerreno`, `fichAreaConstruccion`, `fichFrontis`, `fichDormitorios`, `fichBanios`, `fichCochera`, `fichDescipcion`, `idAsesor`, `fichFechaAuto` FROM `fichas` WHERE idFicha={$_GET['cursor']};";
			...
			?>
				...
				<div class='row p-2 text-center justify-content-center'>
					<h3 class="m-0" id="h3Titulo"><strong>INFORMACIÓN   DE   LA   PROPIEDAD - CÓDIGO: BR-<?= str_pad($_GET['cursor'], 4, 0, STR_PAD_LEFT); ?></strong>
						<?php if (isset($_COOKIE['ckPower']) && $_COOKIE['ckPower'] == 1): ?>
							<a href="editar_ficha.php?cursor=<?= $_GET['cursor']; ?>" class="btn btn-outline-info btn-sm ml-3" title="Editar ficha"><i class="icofont-pencil"></i> Editar</a>
						<?php endif; ?>
					</h3>
				</div>
				...
				<div class="row mb-1 px-3" id="row1">
					...
					<div class="col-7 rounded border border-abregu p-0 mr-1 ml-1"><img src="<?php
																																									...
																																									if (file_exists($foto1 . ".jpg")) {
																																										echo $foto1 . ".jpg?token=" . rand(1000, 99999);
																																									}

																																									if (file_exists($foto1 . ".png")) {
																																										echo $foto1 . ".png?token=" . rand(1000, 99999);
																																									}

																																									if (file_exists($foto1 . ".jpeg")) {
																																										echo $foto1 . ".jpeg?token=" . rand(1000, 99999);
																																									} ?>" class="img-fluid"></div>

					<div class="col-5 rounded border border-abregu p-2 ml-1">
						<div class=" ">
							<p class="float-right py-1 px-3 ml-3" id="pPrecio"> <?= ($row['moneda'] == 'dólares' ? '$' : 'S/.') . ' ' . $row['fichPrecio']; ?></p>
						</div>
				...
				<div class="row mb-1 px-3" id="row2">
					...
					<div class="col-7  p-0 mr-1 ml-1">
						...
						<div class="rounded border border-abregu ">
							<img src="<?php
												...
												if (file_exists($foto2 . ".jpg")) {
													echo $foto2 . ".jpg?token=" . rand(1000, 99999);
												}

												if (file_exists($foto2 . ".png")) {
													echo $foto2 . ".png?token=" . rand(1000, 99999);
												}

												if (file_exists($foto2 . ".jpeg")) {
													echo $foto2 . ".jpeg?token=" . rand(1000, 99999);
												} ?>" class="img-fluid">
						</div>
						<div class="my-1 rounded border border-abregu ">
							...
							<img src="<?php
												...
												if (file_exists($foto3 . ".jpg")) {
													echo $foto3 . ".jpg?token=" . rand(1000, 99999);
												}

												if (file_exists($foto3 . ".png")) {
													echo $foto3 . ".png?token=" . rand(1000, 99999);
												}

												if (file_exists($foto3 . ".jpeg")) {
													echo $foto3 . ".jpeg?token=" . rand(1000, 99999);
												} ?>" class="img-fluid">
					...
					<div class="col-5 my-1 rounded border border-abregu p-2 ml-1 text-justify" id="rowDescripcion">
						...
						<div class="px-4" id="divContenido"><?= $row['fichDescipcion']; ?></div>
					</div>
	...
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalCenterTitle"><i class="bi bi-pencil-square"></i> Editar inmueble</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
...
<script type="text/javascript">
	...
	<?php if ($_COOKIE['ckPower'] == 1): ?>
		...
		function cambiarPrecio(idFicha, precio) {
			$.ajax({
				url: 'returnDetalleInmobiliaria.php',
				type: 'POST',
				data: {
					idFicha: idFicha
				}
				...
				for (let index = 1; index <= 3; index++) {
					...
					if (datos[index].foto != '') {
						...
						$('#divImagenesPost').append(`<div class="row px-3 my-2" id="${index}"><input type="file" data-id="${data.idFicha}" class="form-control" accept="image/*" id="txtFoto${index}" name="txtFoto${index}"><div>`);
					}
		...
		$('#btnActualizar').click(function() {
			$.ajax({
				...
				type: 'POST',
				data: {
					ficha: $('#btnActualizar').attr('data-id'),
					precio: $('#txtNuevoPrecio').val(),
...
<style>
	...
	@media print {
		...
		.divCinta {
			...
			background-color: #C59641 !important;
	...
	#pPrecio {
		border: 6px solid #B12E1B !important;
		color: #B12E1B;
		font-weight: 700;
	...
	*/
	...
	#txtBuscador::placeholder {
		font-family: 'Icofont', -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji"
	}
	...
	/* Quill lists: bullets vs numbered */
	#divContenido ol[data-list="bullet"] {
		list-style-type: disc;
		padding-left: 1.5rem;
	}

	#divContenido ol[data-list="bullet"] li::before {
		display: none;
	}

	#divContenido ol[data-list="ordered"] {
		list-style-type: decimal;
		padding-left: 1.5rem;
	}

	.ajs-message.ajs-error {
		background: rgb(229 1 1 / 95%) !important;
		color: white;
	}
